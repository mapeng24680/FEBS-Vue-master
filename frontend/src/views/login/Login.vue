                ...
                if (this.activeKey === '1') {
                    ...
                    this.form.validateFields(['name', 'password'], (errors, values) => {
                        if (!errors) {
                            ...
                            //考试星登陆，为了拿到cookie
                            //由于我们的系统限制了账号长度，所以要把账号映射出来到考试星
                            this.$post('/login/account/login', {
                                userName: this.loginObjUser[name],
                                userNameInput: `${this.loginObjUser[name]}@175076`,
                                companyId: `175076`,
                                newCompanyId: `175076`,
                                nextUrl: ``,
                                remember: false,
                                password: md5(this.loginObjPwd[name])
                            }).then((r) => {
                                setCookie('sessionId', r.data.bizContent.sessionId);
                                //token接口
                                this.$post('/login/public/login_check', JSON.stringify({sessionId: getCookie('sessionId')})).then((r) => {
                                    this.saveKsx(r.data.data.bizContent)
                                }).catch((e) => {
                                    console.error(e)
                                })
                            }).catch((e) => {
